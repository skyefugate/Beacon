...
class TestMetricsRoutes:
    ...
    def client(self, mock_influx_storage):
        ...
        app = create_app()
        with patch("beacon.api.routes.metrics.get_influx_storage", return_value=mock_influx_storage):
    ...
    def test_query_metrics_large_results(self, client, mock_influx_storage):
        ...
        large_results = [
            {"_measurement": f"metric_{i}", "_value": float(i)}
            for i in range(1000)
        ]
    ...
    def test_query_metrics_special_characters(self, client, mock_influx_storage):
        ...
        query_with_special_chars = 'from(bucket: "test") |> filter(fn: (r) => r.tag == "special/chars@#$%")'
