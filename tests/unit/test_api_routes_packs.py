...
class TestPacksRoutes:
    ...
    @pytest.fixture
    def client(
        self, mock_pack_registry, mock_pack_executor, mock_evidence_builder, mock_evidence_store
    ):
        ...
        with (
            ...
            patch(
                "beacon.api.routes.packs.get_evidence_builder", return_value=mock_evidence_builder
            ),
    ...
    def test_run_pack_already_running(self, client, mock_pack_registry, sample_pack):
        ...
        run_status = {
            existing_run_id: {"status": "running", "pack": "test_pack", "_ts": time.monotonic()}
        }
