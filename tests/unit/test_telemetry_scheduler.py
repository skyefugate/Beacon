...
class TestTelemetryScheduler:
    ...
    @pytest.mark.asyncio
    async def test_sampler_loop_with_interval_override(
        self, mock_settings, mock_sampler, mock_buffer
    ):
        scheduler = TelemetryScheduler(mock_settings, [mock_sampler], mock_buffer)
        scheduler._running = True
        scheduler._interval_overrides["test_sampler"] = 60
    ...
    @pytest.mark.asyncio
    async def test_sampler_loop_sleep_wake_detection(
        self, mock_settings, mock_sampler, mock_buffer
    ):
        scheduler = TelemetryScheduler(mock_settings, [mock_sampler], mock_buffer)
        scheduler._running = True
    ...
    @pytest.mark.asyncio
    async def test_export_loop_exporter_exception(
        self, mock_settings, mock_sampler, mock_buffer, mock_exporter
    ):
        scheduler = TelemetryScheduler(mock_settings, [mock_sampler], mock_buffer, [mock_exporter])
        scheduler._running = True
    ...
    @pytest.mark.asyncio
    async def test_export_loop_exporter_without_flush_buffer(
        self, mock_settings, mock_sampler, mock_buffer
    ):
    ...
    @pytest.mark.asyncio
    async def test_export_windows_exporter_exception(
        self, mock_settings, mock_sampler, mock_buffer, mock_exporter
    ):
        scheduler = TelemetryScheduler(mock_settings, [mock_sampler], mock_buffer, [mock_exporter])
